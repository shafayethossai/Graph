...
const ll N = 2e5 + 7;
ll parent[N], component_size[N];
ll first_connected[N];
...
int main() {
    ...
    for (int i = 0; i < m; i++) {
        Union(roads[i].first, roads[i].second, i+1);
    }
    ...
    while(q--) {
        ...
        if (parent[u] != parent[v]) {
            ...
            cout << max(first_connected[u], first_connected[v]) << endl;
        }
    }
